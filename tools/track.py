...
def main(exp, args, num_gpu):
    ...
    *_, summary = evaluator.evaluate(
    ...
    # else:
    #     if sampling_rate:
    #         gtfiles = glob.glob(os.path.join('datasets/mot_{}/train'.format(sampling_rate), '*/gt/gt{}.txt'.format(gt_type)))
    #     else:
    #         gtfiles = glob.glob(os.path.join('datasets/mot/train', '*/gt/gt{}.txt'.format(gt_type)))
    # print('gt_files', gtfiles)
    # tsfiles = [f for f in glob.glob(os.path.join(results_folder, '*.txt')) if not os.path.basename(f).startswith('eval')]
    # gtfiles = [gtf for gtf in gtfiles if '13' not in gtf]
    # tsfiles = [tsf for tsf in tsfiles if '13' not in tsf]
    ...
    #     fmt[k] = fmt['mota']
    # summary.to_csv("%s/result.csv" % file_name, sep=",")
