...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, did):

        # wait activate
        self._tlwh = np.asarray(tlwh, dtype=np.float)
        self.kalman_filter = None
        self.mean, self.last_mean, self.covariance = None, None, None
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size, frame_id):
        ...
        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, hdi) for
        ...
        if len(dets_second) > 0:
            '''Detections'''
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, lid) for
