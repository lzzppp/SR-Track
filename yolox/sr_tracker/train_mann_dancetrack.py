...
def draw_sequences(predict_sequence, target_sequence, width, height, length, figpath):
    ...
    ax[0].invert_yaxis()  # Reverse the y axis as the origin point is top left in image processing
    ...
    ax[1].invert_yaxis()  # Reverse the y axis as the origin point is top left in image processing
...
def make_dataset(args, det_file, data_flag="train"):
    data_dir = os.path.join("./datasets", args.data_name, data_flag)
...
def batch_sampler(dataset, batch_size, seq_len, samples=10, hiddens=128, eval=False):
    ...
    while True:
        ...
        for frameid in range(start_frame, max_frame, sample_rate):
            ...
            detectids = sorted(detectids, key=lambda tid: scene_data[frameid][tid][0], reverse=True)
            for tid, detectid in enumerate(detectids):
                if tid >= hiddens // 4: break
        ...
        assert len(x) == len(y) and len(x) == seq_len, f"Error: {len(x)} != {len(y)} or {len(x)} != {seq_len}"
        batch_c.append(c)
        batch_x.append(x)
        batch_y.append(y)
        ...
        if len(batch_x) == batch_size:
            if eval: yield batch_c, batch_x, batch_y, batch_lengths, batch_width, batch_height
            else: yield batch_c, batch_x, batch_y, batch_lengths
            batch_c, batch_x, batch_y, batch_lengths = [], [], [], []
...
seed = random.randint(0, 100000) # 50606
...
train_dataset, valid_dataset = make_dataset(args, "gt.txt", data_flag="train"), make_dataset(args, "gt.txt", data_flag="val")
...
for batch_data in batch_sampler(train_dataset, batchsize, seqlength):
    ...
    seq_index = torch.arange(seqlength).unsqueeze(0).expand(batchsize, -1).cuda()
    ...
    loss = loss_func(valid_predict_sequences, valid_target_sequences).sum(dim=1).mean()
    ...
    if train_step % save_step == 0:
        torch.save(model.state_dict(), f'./yolox/sr_tracker/{data_flag}_result/model_{train_step}.pth')
    ...
    if train_step % train_val_step == 0:
        ...
        with torch.no_grad():
            ...
            for batch_data in batch_sampler(valid_dataset, batchsize, seqlength, eval=True):
                ...
                if valid_step > val_steps: break
                
                infos.append([sequences, target_sequences, predict_sequences, widths, heights, raw_lengths])
                valid_step += 1
            ...
            if metric < best_metirc:
                best_metirc = metric
                torch.save(model.state_dict(), f'./yolox/sr_tracker/{data_flag}_result/model_best.pth')
            ...
            for ii, info in enumerate(infos):
                ...
                if not os.path.exists(f'./yolox/sr_tracker/{data_flag}_result/sequences'):
                    os.makedirs(f'./yolox/sr_tracker/{data_flag}_result/sequences')
                
                if not os.path.exists(f'./yolox/sr_tracker/{data_flag}_result/sequences/{train_step}'):
                    os.makedirs(f'./yolox/sr_tracker/{data_flag}_result/sequences/{train_step}')
                ...
                for predict_sequence, target_sequence, width, height, length in zip(predict_sequences, target_sequences, widths, heights, lengths):
                    ...
                    predict_sequence = predict_sequence * np.array([width, height, width, height])
                    target_sequence = target_sequence * np.array([width, height, width, height])
                    draw_sequences(predict_sequence, target_sequence, width, height, length.item(), f'./yolox/sr_tracker/{data_flag}_result/sequences/{train_step}/{ii}_{sequenceid}.png')
                    sequenceid += 1
        
        model.train()
    
    if train_step > train_steps: break