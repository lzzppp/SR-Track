...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, did, frame_id, image, cluster_id=-1):
        ...
        self.mean, self.last_mean, self.covariance, self.last_covariance, self.alpha = None, None, None, None, 1.0
    ...
    def activate(self, kalman_filter, frame_id, video_name, srate):
        ...
        if frame_id == 1: # and '13' not in video_name:
            self.is_activated = True
        # if frame_id == 1 and '13' in video_name and srate < 4:
        #     self.is_activated = True
    ...
    def last_tlwh(self):
        ...
        # ret[2] *= ret[3]
        # ret[:2] -= ret[2:] / 2
        ret = self.xywh2xywh(self.last_mean[:4].copy())
        return ret
...
class DYTETracker(object):
    def __init__(self, args, video_name, frame_rate=30):
        ...
        self.code = args.code
        self.frist_match = args.first_match
    ...
    def update(self, output_results, img_info, img_size, frame_id, raw_img):
        ...
        if len(u_unconfirmed) > 0 and len(u_detection) > 0 and self.args.first_match:
            u_unconfirmed_stracks = [unconfirmed[strack_i] for strack_i in u_unconfirmed]
            u_detections = [detections[detection_i] for detection_i in u_detection]
            
            embeddings = self.encoder.inference_images([strack.last_appearance for strack in u_unconfirmed_stracks] + \
                                                       [sdetection.last_appearance for sdetection in u_detections])
            unconfirmed_appearances = embeddings[:len(u_unconfirmed_stracks)]
            undetection_appearances = embeddings[len(u_unconfirmed_stracks):]

            u_unconfirmed_tlbrs = [strack.tlbr for strack in u_unconfirmed_stracks]
            u_detection_tlbrs = [sdetection.tlbr for sdetection in u_detections]

            ciou_dists = 1.0 - matching.ciou_batch(u_unconfirmed_tlbrs, u_detection_tlbrs)
            appe_dists = cdist(unconfirmed_appearances, undetection_appearances, metric='cosine') / 2.0
            ciou_mask = ciou_dists > self.proximity_thresh
            
            appe_dists[ciou_mask] = 1.0
            matches, final_u_unconfirmed, final_u_detection = matching.linear_assignment(appe_dists, thresh=self.appearance_thresh)

            for strack_i, detection_i in matches:
                u_unconfirmed_stracks[strack_i].update(u_detections[detection_i], self.frame_id)
                activated_starcks.append(u_unconfirmed_stracks[strack_i])
                track.update_last_frame(frame_id)

            for strack_i in final_u_unconfirmed:
                track = u_unconfirmed_stracks[strack_i]
                track.mark_removed()
                removed_stracks.append(track)

            for detection_i in final_u_detection:
                track = u_detections[detection_i]
                if track.score < self.det_thresh:
                    continue

                track.activate(self.kalman_filter, self.frame_id, self.video_name, self.args.sampling_rate)
                activated_starcks.append(track)
        else:
            ...
            # TODO: extract feature of new tracks
